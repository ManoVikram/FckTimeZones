...
function TimeField({ allTimezones, selectedTimezone, time, onTimezoneSelected, onTimeChange, isReadOnly }) {
  return (
    <div className="flex flex-row bg-white h-16 py-4 px-5 rounded-xl justify-between">
      <input type="text" name='from-time' placeholder='00:00 AM/PM' defaultValue={time} className='focus:outline-none text-2xl mr-5' readOnly={isReadOnly} onChange={(event) => onTimeChange(event.target.value)} />
